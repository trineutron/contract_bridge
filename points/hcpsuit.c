...
void getHCP(int count[], int make[][7], int totaltrick[], int totaltrick2[])
{
    ...
    const int hcp_card[4] = { 4, 3, 2, 1 }, dp_length4[14] = { 6, 4, 1 }, dp_length3[14] = { 5, 3, 0 }, dp_length2[14] = { 3, 2, 0 },
	fitpoint[6] = { 0, 1, 2, 2, 2, 2 };
    for (;;) {
        int hcp = 0, spade[2] = { 0, 0 };
        unsigned char trick;
        for (int suit = 0; suit < 4; suit++) {
            int length[2] = { 0, 0 }, hcp_suit[2] = { 0, 0 }, honor = 0, ace = 0, soft = 0;
            ...
            for (int i = 0; i < 4; i++) {
			    ...
			    if (hcp_card[j] != 0 && hcp_card[j] != 4) honor++;
			    if (hcp_card[j] == 4) ace++;
			    if (hcp_card[j] == 1 || hcp_card[j] == 2) soft++;
			}
                    }
                    cache /= 4;
                }
            ...
            for (int side = 0; side < 2; side++) {
                hcp += hcp_suit[side];
		if (length[side] == 2 && soft == 2) hcp--;
                if (length[side] <= 1) hcp -= honor;
		if (length[side] == 0) hcp -= ace;
		if (spade[side] >= 4) {
		    hcp += dp_length4[length[side]];
		} else if (spade[side] == 3) {
		    hcp += dp_length3[length[side]];
		} else if (spade[side] == 2) {
		    hcp += dp_length2[length[side]];
		}
            }
        }
        hcp += fitpoint[spade[0] + spade[1] - 8];
